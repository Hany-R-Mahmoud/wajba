...
export const SettingsView: React.FC<SettingsViewProps> = ({ language, theme, onLanguageChange, onThemeToggle, isRamadanMode, onToggleRamadanMode, onImportBackup, onClearUserData }) => {
  ...
  return (
    <div className="mx-auto max-w-4xl space-y-6">
      ...
      <section className="grid gap-4 sm:grid-cols-2">
        ...
        <div className="rounded-3xl border border-[#d9d9dd] bg-white p-5 dark:border-[#2b3a54] dark:bg-[#162032]"><h2 className="font-bold">{isArabic ? 'وضع التخطيط' : 'Planning mode'}</h2><div className="mt-3 flex gap-2"><button type="button" aria-pressed={!isRamadanMode} onClick={() => { if (isRamadanMode) onToggleRamadanMode(); }} className={`rounded-xl px-4 py-2 text-sm font-bold ${!isRamadanMode ? 'bg-[#17171c] text-white' : 'bg-[#eeece7] dark:bg-stone-800'}`}>{isArabic ? 'الوضع العادي' : 'Standard'}</button><button type="button" aria-pressed={isRamadanMode} onClick={() => { if (!isRamadanMode) onToggleRamadanMode(); }} className={`rounded-xl px-4 py-2 text-sm font-bold ${isRamadanMode ? 'bg-[#17171c] text-white' : 'bg-[#eeece7] dark:bg-stone-800'}`}>{isArabic ? 'وضع رمضان' : 'Ramadan'}</button></div></div>
      </section>
